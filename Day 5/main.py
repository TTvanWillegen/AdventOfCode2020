...
def part_1(advent_of_code):
    with open('input.txt', 'r') as input_file:
        ...
        for line in input_file:
            seats.append(seat_id(decode(line[0:7]), decode(line[7:10])))
        advent_of_code.answer(1, max(seats))
...
def decode(s):
    lower_bound = 0
    upper_bound = 127 if s[0] in ['F', 'B'] else 7
    for char in s:
        if char == 'F' or char == 'L':
            upper_bound = int((upper_bound - lower_bound) / 2 + lower_bound)
        elif char == "B" or char == "R":
            lower_bound = int((upper_bound - lower_bound) / 2 + lower_bound + 1)
    return lower_bound
